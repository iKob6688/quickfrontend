...
export function SalesNotesListPage() {
  ...
  const query = useQuery({
    queryKey: ['salesNotes', { type, dateFrom, dateTo }],
    queryFn: () =>
      listSalesNotes({
        ...(type !== 'all' ? { type } : {}),
        ...(dateFrom ? { dateFrom } : {}),
  ...
  const columns: Column<SalesNoteListItem>[] = useMemo(
    () => [
      {
        key: 'number',
        header: 'เลขที่',
        cell: (r) => (
          <button className="btn btn-link p-0 text-decoration-none" onClick={() => navigate(`/sales/notes/${r.id}`)}>
      ...
      },
      {
        key: 'partner',
        header: 'ลูกค้า',
        cell: (r) => <span>{r.partner?.name || '—'}</span>,
      },
      {
        key: 'date',
        header: 'วันที่เอกสาร',
        cell: (r) => <span className="font-monospace">{r.documentDate || '—'}</span>,
      },
      {
        key: 'total',
        header: 'รวม',
  ...
  return (
    <div>
      <PageHeader
        title="ใบเพิ่ม/ลดหนี้ (Sales)"
        subtitle="Debit Note / Credit Note"
